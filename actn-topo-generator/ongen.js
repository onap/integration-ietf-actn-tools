...
Draw.loadPlugin(function (ui) {
    ...
    const tsNumbers = new Map([["ODU1", 2], ["ODU2", 8], ["ODU3", 32], ["ODU4", 80]])
    const getBitmap = ts => Buffer.alloc(Math.ceil(ts / 8), 255).toString('base64')
    ...
    menu.funct = function (menu, parent) {
        ...
        ui.menus.addMenuItems(menu, [
            '-', 'actnJSON', 'initPNC', 'collapseAll', 'expandAll', 'assignNodeId', 'loadSetting'], parent)
    }

    // Adds action of collpaseAll
    ui.actions.addAction('collapseAll', function () {
        let cells = model.filterDescendants(cell => !!(cell.getAttribute("te_node_id")))
        let nodeAllCollapsed = true
        for (let cell of cells) {
            nodeAllCollapsed &= cell.collapsed
        }
        if (!nodeAllCollapsed) {
    ...
    ui.actions.addAction('expandAll', function () {
        graph.selectVertices(null, true)
    ...
    ui.actions.addAction('loadSetting', function () {
        let plugins = mxSettings.getPlugins()
        mxSettings.parse(DEFAULT_SETTING)
        mxSettings.setPlugins(plugins)
        mxSettings.save()

        ui.setDatabaseItem(null,
            [{ title: '.scratchpad', size: SCRATCHPAD.length, lastModified: Date.now(), type: 'L' },
            { title: '.scratchpad', data: SCRATCHPAD }],
            null, null, ['filesInfo', 'files'])
    ...
    ui.actions.addAction('saveSetting', function () {
        ...
        fs.writeFileSync(ongenDir + "default-settings.json", JSON.stringify(mxSettings.settings))
    })
    ui.keyHandler.bindAction(72, !0, 'saveSetting', !1)

    // Adds action for actn JSON
    ui.actions.addAction('actnJSON', function () {
        let domains = generateJson()
        if (!domains) return
        //Get file base name
        let fileDir = dialog.showOpenDialogSync(win, {
        ...
        for (let domain of domains) {
            let fileName = fileDir + "/" + (domain._port_ || domain.te_name)
        ...
        for (let domain of domains) {
            ...
            const req = http.request(options, (res) => {
                if (res.statusCode != 201 && res.statusCode != 200) {
                    notify(`${res.statusCode} Error from ${options.host}:${options.port}`)
                    return res.resume()
                } else if (!--reqNum) {
    ...
    ui.actions.addAction('assignNodeId', function () {
        let domains = model.filterDescendants(cell => !!(cell.getAttribute("network_types")))
        let i = 1
        graph.clearSelection()
        model.beginUpdate()
        for (let domain of domains) {
            let nodes = model.getChildVertices(domain)
            let j = 1, k = 1
            for (let node of nodes) {
                if (!node.hasAttribute("te_node_id")) {
                    continue
                }
                let newXmlValue = node.value.cloneNode(false)
    ...
    function generateJson() {
        ...
        graph.clearSelection()
        for (let domain of domains) {
            if (!checkObjId("Domain", "te_name", domainMap, domain)) { return }
            let otnTopo = xml2js(domain.value)
            let ethTopo = { ...otnTopo }
            ...
            for (let node of nodes) {
                if (!checkObjId("Node", "te_node_id", nodeMap, node)) { return }
                ...
                for (let tp of tps) {
                    ...
                    if (newTpLink["otn_odu_type"] && links.length == 1) {
                        ...
                        if (dest_tp.length != 0) {
                            if (newTpLink["otn_odu_type"] != dest_tp[0].getAttribute("otn_odu_type")) {
                                notify("The two TPs of the link don't have the same odu_type!")
                                graph.addSelectionCell(tp)
                            ...
                            if (dest_node != null && dest_node != undefined && node.parent == dest_node.parent) {
            ...
            let jsonData = jjsRender(ONGEN_TMPL,
                { networks: [otnTopo, ethTopo], btoa: btoa })
    ...
    function xml2js(xmlNode, jsObj = {}) {
        for (attr of xmlNode.attributes) {
            if (attr.name != "label" && attr.name != "placeholders") {
                attr.value = attr.value.replace(/(^\s*)|(\s*$)/g, "")
    ...
    function checkObjId(objName, objIdName, objMap, newObj) {
        ...
        if (!newObjValue) {
            notify(objName + " " + objIdName + " is missing!")
            graph.addSelectionCell(newObj)
            return false
        }
        if (objMap.has(newObjValue)) {
            notify("Same " + objName + " " + objIdName + "!")
            graph.addSelectionCell(newObj)
                        ...
                        "ietf-te-topology:te": {
                            ...
                            "tunnel-termination-point": [
                                << let $i = 0; node.tps.forEach((tp, i)=> { >>
                                    << $i ? ',' : '' >>
                                << if (tp.otn_client_facing == "false") { $i = 1 >>
                                {
